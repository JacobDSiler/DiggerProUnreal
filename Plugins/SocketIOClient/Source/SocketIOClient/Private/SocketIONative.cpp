...
void FSocketIONative::Connect(const FSIOConnectParams& InConnectParams)
{
	...
	FCULambdaRunnable::RunLambdaOnBackGroundThread([&, StdAddressString, StdPathString, QueryMap, HeadersMap, AuthMessage]
	{
		if (!PrivateClient)
		{
			UE_LOG(SocketIO, Error, TEXT("PrivateClient is null before attempting connection."));
			return;
		}
		...
		try
		{
			...
			if (PrivateClient->opened())
			{
				...
				{
					UE_LOG(SocketIO, Warning, TEXT("Already connected to %s. Skipping connect."),
					       UTF8_TO_TCHAR(StdAddressString.c_str()));
					return;
				}
			}
		...
		{
			UE_LOG(SocketIO, Error, TEXT("Exception during socket connect: %s"), UTF8_TO_TCHAR(e.what()));
		}
	});
}
